...
export default Ember.Component.extend(HasBlockMixin, {
  ...
  filterArray: function filterArray(array, searchText, lookupKey) {
    return array.filter(function (item) {
      Ember.assert('You have not defined \'lookupKey\' on paper-autocomplete, when source contained ' + 'items that are not of type String. To fix this error provide a ' + 'lookupKey=\'key to lookup from source item\'.', isString(item) || Ember.isPresent(lookupKey));

      Ember.assert('You specified \'' + lookupKey + '\' as a lookupKey on paper-autocomplete, ' + 'but at least one of its values is not of type String. To fix this error make sure that every \'' + lookupKey + '\' value is a string.', isString(item) || Ember.isPresent(lookupKey) && isString(Ember.get(item, lookupKey)));
